...
void keccak_hash(uint8_t *input, size_t input_len, uint8_t *output, size_t output_len) {
    ...
    // Ensure proper alignment using posix_memalign
    if (posix_memalign((void**)&state, 32, sizeof(KeccakP1600_AVX2_state)) != 0) {
        fprintf(stderr, "Memory allocation failed: %s\n", strerror(errno));
        exit(EXIT_FAILURE);
    }

    printf("Initializing Keccak state...\n");

    // Perform the Keccak permutation (12 or 24 rounds)
...
int main(void) {
    ...
    for (int si = 0; si < (int)(sizeof(sizes) / sizeof(*sizes)); si++) {
        ...
        // Ensure alignment using posix_memalign
        if (posix_memalign((void**)&input, 32, N) != 0 || posix_memalign((void**)&output, 32, 64) != 0) {
            fprintf(stderr, "Memory allocation failed: %s\n", strerror(errno));
            exit(EXIT_FAILURE);
        }
