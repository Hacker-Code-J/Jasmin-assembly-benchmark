...
/*
 * Compute SHA3-256(input, input_length) → hash[32].
 * Returns 1 on success, 0 on error.
 */
int ossl_hash_sha3_256_amd64_ref(
    uint8_t *hash,
    const uint8_t *input,
    ...
    /* init for SHA3: pad=0x06, digest length=256 bits */
    if (!ossl_sha3_init(&ctx, 0x06 /* SHA3 pad */, 256))
        return 0;

    /* absorb the entire input */
    if (!ossl_sha3_update(&ctx, input, (size_t)input_length))
        return 0;

    /* squeeze out the 32-byte (256-bit) digest */
    if (!ossl_sha3_final(&ctx, hash, 32))
        return 0;
