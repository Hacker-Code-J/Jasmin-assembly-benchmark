...
{
  int r;
  uint8_t hash[OSSL_HASH_sha3_256_amd64_ref_BYTES];
  uint8_t input[] = {0x61, 0x62, 0x63};

  //
  r = ossl_hash_sha3_256_amd64_ref(hash, input, sizeof(input));
    assert(r == 0);

  print_info(OSSL_HASH_sha3_256_amd64_ref_ALGNAME, OSSL_HASH_sha3_256_amd64_ref_ARCH, OSSL_HASH_sha3_256_amd64_ref_IMPL);
  print_str_u8("input", input, sizeof(input));
  print_str_u8("hash", hash, OSSL_HASH_sha3_256_amd64_ref_BYTES);
