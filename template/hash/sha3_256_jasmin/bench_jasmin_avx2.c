// gcc -std=c11 -O2 -Wall -maes -mssse3 -mavx bench_jasmin_avx2.c sha3_256_avx2.S -o bench_jasmin_avx2 -lm
...
extern int jade_hash_sha3_256_amd64_avx2(
    uint8_t *hash,
    const uint8_t *input,
...
int main(void) {
    ...
    for (int si = 0; si < NSZ; si++) {
        ...
        /* warm-up: */
        for (int w = 0; w < 10; w++)
            jade_hash_sha3_256_amd64_avx2(hash, in, N);
